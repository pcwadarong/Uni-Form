    ...
    <div className="bg-white m-y-auto w-full max-w-[1200px] shadow px-14 md:pt-20 pb-10 space-y-20">
      <section className="flex flex-col md:flex-row gap-10">
        <div className="md:hidden w-screen relative left-1/2 -translate-x-1/2 shadow overflow-hidden">
          {/* {item.img && <Image src={item.img} width={240} height={150} alt={"form 이미지"} className="md:hidden w-full border shadow -mx-14"/>} */}
          <Image
            src="/preview.jpg"
            width={240}
            height={150}
        ...
        <aside className="flex md:flex-col w-full md:w-60 gap-3">
          {/* {item.img && <Image src={item.img} width={240} height={150} alt={"form 이미지"} className="hidden md:flex border border-gray-300 rounded-md"/>} */}
          {
            <Image
              src={"/preview.jpg"}
              width={240}
          ...
          {item.isPublic && (
            <LinkButton className={"bg-green-100 text-green-500 flex-1"} href={`/analyze/${encryptedId}`}>
              결과보기
            </LinkButton>
          )}
          {diffTime && (
            <LinkButton className={"bg-green-400 text-white flex-1"} href={`/response/${encryptedId}`}>
              참여하기
            </LinkButton>
          )}
